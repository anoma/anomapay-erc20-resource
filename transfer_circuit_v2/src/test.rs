...
fn test_transfer_v2() {
    ...
    let discovery_ciphertext =
        Ciphertext::from_words(&logic_proof::get_instance(&proof).unwrap().app_data.discovery_payload[0].blob);
    discovery_ciphertext.decrypt(&created_discovery_sk).unwrap();

    // check encryption
    let encryption_ciphertext =
        Ciphertext::from_words(&logic_proof::get_instance(&proof).unwrap().app_data.resource_payload[0].blob);
